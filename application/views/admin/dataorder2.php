                        ...
                        <div class="card-header">
                            ...
                            <div class="card-body table-responsive">
                                <table id="dataTable" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        <?php $no = 1;
                                        foreach ($getDataOrderIdadmin2->result() as $row) { ?>
                                            <tr>
                                                <td><?php echo $no; ?></td>
                                                ...
                                                <td>
                                                    <?php if ($row->status == '1') { ?>
                                                        <a href="#" class="btn btn-danger btn-sm  elevation-1" data-toggle="modal" data-target="#konfir" data-id="<?= $row->id_order; ?>">
                                                            Konfirmasi
                                                        </a>
                                                    <?php } else if ($row->status == '2') { ?>
                                                        <a href="DataOrder/KonfirmasiSelesaiM/<?= $row->id_order; ?>/<?= $row->idPkursi; ?>" class="btn btn-danger btn-sm  elevation-1">
                                                            Konfirmasi Selesai
                                                        </a>
                                                    <?php } else if ($row->status == '3') { ?>
                                                        <p style="color:green;">Selesai</p>
    ...
    <div class="modal fade" id="konfir" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="DataOrder/KonfirmasiM" method="POST">
                    <div class="modal-body isikonfir">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button>
                        <button style="display:none;" type="submit" class="btn btn-primary btn-sm" name="proses" id="tombol">Submit</button>
                    </div>
                </form>
            </div>
        </div>
